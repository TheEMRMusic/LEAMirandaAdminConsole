...
function draw() {
  ...
  if (filled) {
    ...
    textAlign(CENTER);
    text("L O N G", (windowWidth / 2), ((windowHeight/1.6)));
    stroke(234, 114, 140);
    strokeWeight(2);
    textSize(windowWidth/20);
    textAlign(CENTER);
    text("LOCATION: Los Angeles, CA", (windowWidth / 2), ((windowHeight / 1.4)));
    textSize(windowWidth/25);
    textAlign(CENTER);
    stroke(241, 225, 200);
    strokeWeight(1.5);
    text("\\\\\\\\ USER DATA LOGS \\\\\\\\", (windowWidth / 2), ((windowHeight / 2) + (windowHeight/3)));
    text("\\\\\\\\ Video and Audio Available for Review \\\\\\\\", (windowWidth / 2), ((windowHeight / 2) + (windowHeight/2.5)));
    stroke(241, 225, 200);
    strokeWeight(2);
    fill(241, 225, 200);
    textAlign(CENTER);
  ...
  // margin border
  translate(((windowWidth/20)), ((windowHeight / 2.5)));

  if (textTyped.length > 0) {
    // get a path from OpenType.js
    var fontPath = font.getPath(textTyped, 0, windowHeight/40, windowWidth/5);
    ...
    // map mouse axis
    var addToAngle = map((mouseX)/2, 0.2, width, -PI, PI/2);
    var curveHeight = map(mouseY, 0.25, width, -PI, PI/2);
